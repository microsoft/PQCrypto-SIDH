...
*
* Abstract: API header file for P503 using compression with ciphertext vs decaps. speed tradeoff
*********************************************************************************************/  

#ifndef P503_COMPRESSED_SPEED_API_H
#define P503_COMPRESSED_SPEED_API_H
...
// Algorithm name
#define CRYPTO_ALGNAME "SIKEp503_compressed_speed"  

// SIKE's key generation
// It produces a private key sk and computes the public key pk.
// Outputs: secret key sk (CRYPTO_SECRETKEYBYTES = 407 bytes)
//          public key pk (CRYPTO_PUBLICKEYBYTES = 225 bytes) 
int crypto_kem_keypair_SIKEp503_compressed_speed(unsigned char *pk, unsigned char *sk);
...
int crypto_kem_enc_SIKEp503_compressed_speed(unsigned char *ct, unsigned char *ss, const unsigned char *pk);

// SIKE's decapsulation
// Input:   secret key sk         (CRYPTO_SECRETKEYBYTES = 407 bytes)
//          ciphertext message ct (CRYPTO_CIPHERTEXTBYTES = 280 bytes) 
// Outputs: shared secret ss      (CRYPTO_BYTES = 24 bytes)
int crypto_kem_dec_SIKEp503_compressed_speed(unsigned char *ss, const unsigned char *ct, const unsigned char *sk);
...
// Ciphertexts ct consist of the concatenation of 4 values of length OALICE_BITS, one element in GF(p503^2), 2 bytes and a 24-byte value. In the SIKE API, 
...
int EphemeralKeyGeneration_A_SIDHp503_Compressed_Speed(unsigned char* PrivateKeyA, unsigned char* PublicKeyA);
...
int EphemeralKeyGeneration_B_SIDHp503_Compressed_Speed(const unsigned char* PrivateKeyB, unsigned char* PublicKeyB);
...
int EphemeralSecretAgreement_A_SIDHp503_Compressed_Speed(const unsigned char* PrivateKeyA, const unsigned char* PublicKeyB, unsigned char* SharedSecretA, unsigned char* phiBKA_t);
...
int EphemeralSecretAgreement_B_SIDHp503_Compressed_Speed(const unsigned char* PrivateKeyB, const unsigned char* PublicKeyA, unsigned char* SharedSecretB);


// Encoding of keys for KEX-based isogeny system "SIDHp503_compressed_speed" (wire format):
// ----------------------------------------------------------------------
// Elements over GF(p503) are encoded in 63 octets in little endian format (i.e., the least significant octet is located in the lowest memory address). 
// Elements (a+b*i) over GF(p503^2), where a and b are defined over GF(p503), are encoded as {a, b}, with a in the lowest memory portion.
//
