...
// Algorithm name
#define CRYPTO_ALGNAME "SIKEp610_compressed_speed"  

// SIKE's key generation
// It produces a private key sk and computes the public key pk.
// Outputs: secret key sk (CRYPTO_SECRETKEYBYTES = 491 bytes)
//          public key pk (CRYPTO_PUBLICKEYBYTES = 274 bytes) 
int crypto_kem_keypair_SIKEp610_compressed_speed(unsigned char *pk, unsigned char *sk);
...
int crypto_kem_enc_SIKEp610_compressed_speed(unsigned char *ct, unsigned char *ss, const unsigned char *pk);

// SIKE's decapsulation
// Input:   secret key sk         (CRYPTO_SECRETKEYBYTES = 491 bytes)
//          ciphertext message ct (CRYPTO_CIPHERTEXTBYTES = 336 bytes) 
// Outputs: shared secret ss      (CRYPTO_BYTES = 24 bytes)
int crypto_kem_dec_SIKEp610_compressed_speed(unsigned char *ss, const unsigned char *ct, const unsigned char *sk);
...
int EphemeralKeyGeneration_A_SIDHp610_Compressed_Speed(unsigned char* PrivateKeyA, unsigned char* PublicKeyA);
...
int EphemeralKeyGeneration_B_SIDHp610_Compressed_Speed(const unsigned char* PrivateKeyB, unsigned char* PublicKeyB);
...
int EphemeralSecretAgreement_A_SIDHp610_Compressed_Speed(const unsigned char* PrivateKeyA, const unsigned char* PublicKeyB, unsigned char* SharedSecretA, unsigned char* phiBKA_t);
...
int EphemeralSecretAgreement_B_SIDHp610_Compressed_Speed(const unsigned char* PrivateKeyB, const unsigned char* PublicKeyA, unsigned char* SharedSecretB);
